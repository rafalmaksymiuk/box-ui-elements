...
type Props = {
    ...
    // Router-disabled specific props
...
// Router-disabled version that uses internal navigation instead of React Router
class SidebarRouterDisabled extends React.Component<Props, State> {
    static defaultProps = {
        annotatorState: {},
        isDefaultOpen: true,
        isLoading: false,
    ...
    componentDidMount() {
        const { file, api, metadataSidebarProps, docGenSidebarProps, onOpenChange = noop, sidebarNavigation }: Props = this.props;
        
        // If sidebarNavigation prop is provided, use it to initialize internal state
        if (sidebarNavigation && !isEqual(this.state.internalSidebarNavigation, sidebarNavigation)) {
    ...
    internalSidebarNavigationHandler: InternalSidebarNavigationHandler = (newNavigation, replace) => {
        const currentNavigation = this.state.internalSidebarNavigation;
        
        // Check if navigation has changed using lodash isEqual
        if (!isEqual(currentNavigation, newNavigation)) {
            this.setState({ internalSidebarNavigation: newNavigation });
                    ...
                    <>
                        {hasNav && (
                            <SidebarNav
                                ...
                                internalSidebarNavigation={this.state.internalSidebarNavigation}
                                internalSidebarNavigationHandler={this.internalSidebarNavigationHandler}
                                isOpen={isOpen}
...
// Original Sidebar class for router-enabled version
class SidebarRouterEnabled extends React.Component<Props, State> {
    static defaultProps = {
        annotatorState: {},
        isDefaultOpen: true,
        isLoading: false,
